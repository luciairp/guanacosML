...
# hn uniform y hazard rate para calcular individuos usando size dht_group F
...
# version calculando densidad de grupos
# grupos (no individuos) dht_group T
fit.hn.uni.haz.list.group <- function(data, trunc) {
  ...
  conversion.factor <- convert_units("Meter", "Kilometer", "Square kilometer")
  models <- list()
  models$hn.cos <- ds(data, trun=trunc, key="hn", adj="cos", dht_group = T,
                      max_adjustments = 3, monotonicity = "strict",
                      convert_units = conversion.factor)
  models$uni.cos <- ds(data, trun=trunc, key="unif", adj="cos", dht_group = T,
                       max_adjustments = 3, monotonicity = "strict",
                       convert_units = conversion.factor)
  models$haz.poly <- ds(data, trun=trunc, key="hr", adj="poly", dht_group = T,
                        max_adjustments = 3, monotonicity = "strict",
                        convert_units = conversion.factor)
...
extract_model_res_table <- function(model){
  ...
  out <- as_tibble(matrix(0, nrow = 1, ncol = 16))
  colnames(out) <-c('Modelo','Región','AreaCubierta',
                    'D_est','SE','Lcl','Ucl',
                    'Group_Size_est','SE_grupo',
                    'N_est','LclN','UclN',
                    'gr_est','gr_se','gr_lcl','gr_ucl')
  out[[1]] <- model$call$key
  out[[2]] <- model$dht$clusters$summary$Region
  out[[3]] <- model$dht$clusters$summary$CoveredArea
  out[[4]] <- model$dht$individuals$D$Estimate
  out[[5]] <- model$dht$individuals$D$se
  out[[6]] <- model$dht$individuals$D$lcl
  out[[7]] <- model$dht$individuals$D$ucl
  out[[8]] <- model$dht$Expected.S$Expected.S
  out[[9]] <- model$dht$Expected.S$se.Expected.S
  out[[10]] <- model$dht$individuals$N$Estimate
  out[[11]] <- model$dht$individuals$N$lcl
  out[[12]] <- model$dht$individuals$N$ucl
  out[[13]] <- model$dht$clusters$N$Estimate
  out[[14]] <- model$dht$clusters$N$se
  out[[15]] <- model$dht$clusters$N$lcl
  out[[16]] <- model$dht$clusters$N$ucl
  return(out)  
}

procesar_muestreo <- function(data, muestreo_valor, especie, 
                              truncado_valor,usar_grupo = TRUE){
  trans <- data %>% 
  ...
  # Selecciona la función a usar según el valor de 'usar_grupo'
  if (usar_grupo) {
    res <- fit.hn.uni.haz.list.group(trans, truncado_valor)
  } else {
    res <- fit.hn.uni.haz.list(trans, truncado_valor)
  }
